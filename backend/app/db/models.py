...
class MaterialTuberia(str, PyEnum):
...
class Proyecto(Base):
    ...
    __table_args__ = (
        Index("idx_proyectos_usuario_id", "usuario_id"),
        Index("idx_proyectos_usuario_updated", "usuario_id", "updated_at"),
    )
    ...
    # Relaciones
    nudos = relationship(
        "Nudo", back_populates="proyecto", cascade="all, delete-orphan"
    )
    tramos = relationship(
        "Tramo", back_populates="proyecto", cascade="all, delete-orphan"
    )
    calculos = relationship(
        "Calculo", back_populates="proyecto", cascade="all, delete-orphan"
    )
...
class Nudo(Base):
    ...
    # Geometría
    coordenadas = Column(
        Geometry(geometry_type="POINT", srid=int(settings.DEFAULT_CRS.split(":")[1])),
        nullable=True,
    )
    ...
    tramos_origen = relationship(
        "Tramo", foreign_keys="Tramo.nudo_origen_id", back_populates="nudo_origen"
    )
    tramos_destino = relationship(
        "Tramo", foreign_keys="Tramo.nudo_destino_id", back_populates="nudo_destino"
    )
...
class Tramo(Base):
    ...
    geometria = Column(
        Geometry(
            geometry_type="LINESTRING", srid=int(settings.DEFAULT_CRS.split(":")[1])
        ),
        nullable=True,
    )
    ...
    nudo_origen = relationship(
        "Nudo", foreign_keys=[nudo_origen_id], back_populates="tramos_origen"
    )
    nudo_destino = relationship(
        "Nudo", foreign_keys=[nudo_destino_id], back_populates="tramos_destino"
    )
    iteraciones = relationship(
        "Iteracion", back_populates="tramo", cascade="all, delete-orphan"
    )
