...
Endpoints para integración con servicios de mapas y cotas DEM con autenticación
"""
...
async def obtener_cotas_dem(
    ...
    nudos_sin_cota = [
        n for n in nudos if not n.cota_terreno and n.longitud and n.latitud
    ]
    ...
    coordenadas = [
        {"latitude": n.latitud, "longitude": n.longitud} for n in nudos_sin_cota
    ]

    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(
                settings.ELEVATION_API_URL, json={"locations": coordenadas}
            )
...
async def interpolar_cotas(
    ...
    for nudo in nudos_sin_cota:
        cota_interpolada = _interpolar_idw(nudo.longitud, nudo.latitud, nudos_con_cota)
...
def _interpolar_idw(
    lon: float, lat: float, nudos_referencia: List[Nudo], power: float = 2
) -> Optional[float]:
    """
    ...
    for nudo in nudos_referencia:
        ...
        # Calcular distancia
        distancia = _calcular_distancia(lon, lat, nudo.longitud, nudo.latitud)
