...
Endpoints para optimización de diámetros con Algoritmo Genético
"""
...
from app.db.models import (
    Proyecto, Tramo, Nudo, Optimizacion
)
...
async def optimizar_diametros(
    ...
    costo_original = 0.0
    for tramo in tramos_db:
        factor_costo = (tramo.diametro_interior / settings.DIAMETROS_COMERCIALES[0]) ** 1.5
    ...
    optimizacion_db = Optimizacion(
        ...
        ahorro_porcentaje=((costo_original - resultados["costo_total"]) / costo_original * 100) if costo_original > 0 else 0,
    ...
    return OptimizacionResponse(
        ...
        diametros_propuestos={
            str(k): v for k, v in diametros_propuestos.items()
        },
        ...
        if str(tramo.id) in diametros_opt:
            ...
            if abs(diametro_opt - diametro_actual) > 1:
                ...
                recomendaciones.append({
                    ...
                    "accion": cambio,
                    "porcentaje_cambio": round((diferencia / diametro_actual) * 100, 1)
                })
    
    return {
        "recomendaciones": recomendaciones,
        "total_cambios": len(recomendaciones),
        "convergencia": optimizacion.convergencia,
        "costo_optimizado": optimizacion.costo_optimizado
    }
