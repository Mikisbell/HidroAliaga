...
Endpoints para exportación de reportes y archivos con autenticación
"""
...
from app.db.models import Proyecto, Nudo, Tramo, Calculo, Optimizacion
            ...
            "cuadro_nudos": [
                {
                    ...
                    "coordenadas": {"lat": n.latitud, "lng": n.longitud},
                }
                for n in nudos
            ],
            "cuadro_tramos": [
                {
                    "codigo": t.codigo,
                    "nudo_origen": next(
                        (n.codigo for n in nudos if n.id == t.nudo_origen_id), None
                    ),
                    "nudo_destino": next(
                        (n.codigo for n in nudos if n.id == t.nudo_destino_id), None
                    ),
                    "longitud": t.longitud,
...
def _generar_memoria_descriptiva(
    proyecto: Proyecto, nudos: List[Nudo], tramos: List[Tramo], calculo: Calculo
) -> str:
...
async def exportar_excel(
    ...
    data = {
        "proyecto": {
            ...
            "ambito": proyecto.ambito.value
            if hasattr(proyecto.ambito, "value")
            else proyecto.ambito,
            "tipo_red": proyecto.tipo_red.value
            if hasattr(proyecto.tipo_red, "value")
            else proyecto.tipo_red,
...
async def exportar_epanet(
    ...
    for nudo in nudos:
        if (
            nudo.tipo.value == "reservorio"
            if hasattr(nudo.tipo, "value")
            else nudo.tipo == "reservorio"
        ):
            elev = nudo.elevacion or 0
    ...
    for tramo in tramos:
        ...
        lines.append(
            f"{tramo.codigo}\t{tramo.nudo_origen_id}\t{tramo.nudo_destino_id}\t{long:.2f}\t{diam_inch:.3f}\t{roughness:.4f}\t0\tOpen"
        )
...
async def exportar_geojson(
    ...
    for tramo in tramos:
        ...
        if (
            nudo_origen
            and nudo_destino
            and nudo_origen.longitud
            and nudo_destino.longitud
        ):
            features.append(
                {
                    "type": "Feature",
    ...
    geojson = {
        ...
        "features": features,
        "crs": {"type": "name", "properties": {"name": settings.DEFAULT_CRS}},
    }
